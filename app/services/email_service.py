...
def send_otp_email(to_email: str, full_name: str, otp: str) -> None:
    ...
    try:
        # Port 587 = STARTTLS (plain connection, then upgrade to TLS)
        with smtplib.SMTP(settings.SMTP_HOST, settings.SMTP_PORT, timeout=10) as server:
            server.ehlo()
            # Only call starttls on non-Mailpit ports (Mailpit uses port 1025 without TLS)
            use_starttls = settings.SMTP_PORT not in (1025,)
            if use_starttls:
                server.starttls()
                server.ehlo()
            # Only login if credentials are provided (Mailpit doesn't need them)
